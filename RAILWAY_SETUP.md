...
**重要**：Railway会自动将PostgreSQL的环境变量注入到主服务（supercoordination-mcp），无需手动配置。
...
添加PostgreSQL完成后，请告诉我，我将立即执行：

1. **生成Prisma客户端**：`npm run prisma:generate`
2. **运行数据库迁移**：`npm run prisma:migrate:deploy`
3. **导入现有数据**：`npm run migrate:data`
4. **验证迁移结果**：检查数据完整性
