...
const ISSUE_TICKET_COST = parseInt(process.env.ISSUE_TICKET_COST || '5', 10);
const TICKET_VALIDITY_DAYS = parseInt(process.env.TICKET_VALIDITY_DAYS || '30', 10);
...
async function issueTicket(issuerId, recipientEmail) {
  ...
  // 检查发起者积分是否足够
  if (issuer.pointsBalance < ISSUE_TICKET_COST) {
    throw new Error(`积分不足（需要${ISSUE_TICKET_COST}积分）`);
  }
  ...
  await prisma.user.update({
    where: { id: issuerId },
    data: { pointsBalance: { decrement: ISSUE_TICKET_COST } }
  });
  ...
  expiresAt.setDate(expiresAt.getDate() + TICKET_VALIDITY_DAYS); // 默认30天有效期
